...
export const isEnhancedInvoice = (invoice: Invoice): boolean => {
  return !!(invoice.client_info || invoice.amounts || invoice.blockchain_data);
};
...
export const isEnhancedPayment = (payment: Payment): boolean => {
  return !!(payment.network || payment.token_address || payment.confirmed_at);
};
