...
export interface Config {
  ...
  blockchain: {
    // Dynamic blockchain configs now loaded from database
 ...
 */
function isValidSqsUrl(str: string): boolean {
...
if (process.env.NODE_ENV !== 'test') {
  console.log('Fluxion Main Lambda Configuration Loaded:', {
    ...
    redis: {
      ...
      cacheEnabled: config.features.enableRedisCache,
    },
    features: {
      rowLevelSecurity: config.features.enableRowLevelSecurity,
      auditLogging: config.features.enableAuditLogging,
      redisCache: config.features.enableRedisCache,
    },
