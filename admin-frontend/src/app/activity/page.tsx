  ...
  const fetchData = async (page = 1, limit = 50) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        throw new Error(response.error?.message || 'Failed to fetch activity logs')
      }
    } catch (err) {
  ...
  const columns: Column<AuditLog>[] = [
    {
      ...
      render: (value) => {
        try {
          ...
          console.error('Error rendering column: created_at', error)
          return <div className="text-sm text-red-500">Date error</div>
        }
      },
      width: '32'
    },
                    ...
                    {(() => {
                      try {
                        if (!selectedLog.created_at) {
                          return <p className="text-sm text-gray-500">No date available</p>
                        }
                        
                        const date = new Date(selectedLog.created_at)
                        if (isNaN(date.getTime())) {
                        ...
                        console.error('Error rendering date in modal:', error)
                        return <p className="text-sm text-red-500">Date error</p>
                      }
                    })()}
                  </div>
                </div>
