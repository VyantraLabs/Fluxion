...
export default function OrganizationActivityPage() {
  ...
  const [selectedActivity, setSelectedActivity] = useState<OrganizationActivity | null>(null)
  ...
  const fetchData = async (page = 1, limit = 50) => {
    ...
    try {
      ...
      const activityResponse = await adminApi.getOrganizationActivity(organizationId, params)
      
      if (activityResponse.success) {
        setActivities(activityResponse.data.logs || [])
        setPagination(prev => ({
          ...prev,
          page,
          limit,
          total: activityResponse.data.pagination?.total || 0,
          pages: Math.ceil((activityResponse.data.pagination?.total || 0) / limit),
          has_next: page * limit < (activityResponse.data.pagination?.total || 0),
          has_prev: page > 1
        }))
        setError(null)
      } else {
        throw new Error(activityResponse.error?.message || 'Failed to fetch activity')
      }
    } catch (err) {
  ...
  const handleViewActivityDetails = (activity: OrganizationActivity) => {
  ...
  const columns: Column<OrganizationActivity>[] = [
    {
      key: 'created_at',
      title: 'Time',
      render: (value) => (
      ...
      render: (value, activity) => (
        <div className="flex items-center space-x-2">
          ...
          <div>
            ...
            {activity.tableName && (
              <div className="text-sm text-gray-500">Table: {activity.tableName}</div>
            )}
          </div>
        </div>
      ),
    ...
    },
    {
      ...
      render: (value, activity) => (
        <div className="text-sm">
          <div className="font-medium text-gray-900">
            {activity.userName || value || 'Unknown User'}
          </div>
          {value && (
            <div className="text-gray-500">{value}</div>
          )}
          {activity.userId && (
            <div className="text-gray-400 text-xs">ID: {activity.userId.slice(0, 8)}...</div>
          )}
        </div>
      ),
      width: '32'
    ...
    },
    {
      ...
      render: (value, activity) => (
        <div className="text-sm max-w-64">
          ...
          {activity.ipAddress && (
            <div className="text-gray-500 text-xs">IP: {activity.ipAddress}</div>
          )}
        </div>
      ),
      width: '64'
  ...
  const hasActiveFilters = searchTerm || actionFilter || severityFilter || startDate || endDate
          ...
          <div className="admin-card p-6 mb-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-gray-500">Activity Log</p>
              </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center space-x-2">
                        ...
                        {getSeverityBadge(selectedActivity.severityLevel)}
                      </div>
                      {selectedActivity.tableName && (
                        <p className="text-sm text-gray-600">Table: {selectedActivity.tableName}</p>
                      )}
                      {selectedActivity.recordId && (
                        <p className="text-sm text-gray-600">Record ID: {selectedActivity.recordId}</p>
                      )}
                    </div>
                  </div>
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    {selectedActivity.userName && (
                      <p className="text-sm text-gray-900">Name: {selectedActivity.userName}</p>
                    )}
                    {selectedActivity.userEmail && (
                      <p className="text-sm text-gray-900">Email: {selectedActivity.userEmail}</p>
                    )}
                    {selectedActivity.userId && (
                      <p className="text-sm text-gray-600">ID: {selectedActivity.userId}</p>
                    )}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {selectedActivity.ipAddress && (
                      <p className="text-sm text-gray-600">IP Address: {selectedActivity.ipAddress}</p>
                    )}
                    {selectedActivity.userAgent && (
                      <p className="text-sm text-gray-600">User Agent: {selectedActivity.userAgent}</p>
                    )}
                  </div>
                </div>

                {/* Changes Summary */}
                {selectedActivity.changesSummary && (
                  <div>
